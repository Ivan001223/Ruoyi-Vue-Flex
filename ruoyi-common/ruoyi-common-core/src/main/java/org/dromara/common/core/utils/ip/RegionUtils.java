...
public class RegionUtils {
    ...
    static {
        try {
            ...
            // 一般而言，更建议把xdb数据库放到一个指定的文件目录中（即不打包进jar包中），然后使用 NoCache +
            // 配合SearcherPool的并发池读取数据，更方便随时更新xdb数据库

            // TODO 2025年12月23日 Ip2Region封装的 InputStream 读取函数
            // Searcher.loadContentFromInputStream
            // 在Linux环境下会申请过大的byte[]空间而导致OOM，这里先用临时文件的方案解决，等后续 Ip2Region 更新解决方案
            // 创建临时文件
            File v4TempXdb = FileUtil.writeFromStream(ResourceUtil.getStream(DEFAULT_IPV4_XDB_PATH),
            ...
            try {
                ...
                if (v6XdbInputStream == null) {
                    ...
                    // 创建临时文件
                    File v6TempXdb = FileUtil.writeFromStream(ResourceUtil.getStream(DEFAULT_IPV6_XDB_PATH),
                            FileUtil.createTempFile());
